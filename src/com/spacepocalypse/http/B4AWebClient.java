...
public class B4AWebClient {
	...
	public static synchronized boolean insertMappedBeerRating(MappedBeerRating rating) throws NotInitializedException, JSONException {
		...
		instanceHttpClient.addParam(AndroidBeerQueryServlet.KEY_QUERY, AndroidBeerQueryServlet.QUERY_TYPE_INSERT);
		instanceHttpClient.addParam(AndroidBeerQueryServlet.KEY_MAPPED_RATING, new JSONObject(rating).toString());
	...
	public static synchronized boolean insertMappedBeer(MappedBeer beer) throws NotInitializedException, JSONException {
		...
		HttpRestClient instanceHttpClient = instance.httpClient;
		instanceHttpClient.clearParams();
		instanceHttpClient.setServiceExtension(instance.getContext().getString(R.string.service_name_beer));
		instanceHttpClient.addParam(AndroidBeerQueryServlet.KEY_QUERY, AndroidBeerQueryServlet.QUERY_TYPE_INSERT);
		instanceHttpClient.addParam(AndroidBeerQueryServlet.KEY_MAPPED_BEER, new JSONObject(beer).toString());
	...
	public static synchronized boolean updateMappedBeerRating(MappedBeerRating rating) throws NotInitializedException, JSONException {
		...
		instanceHttpClient.addParam(AndroidBeerQueryServlet.KEY_QUERY, AndroidBeerQueryServlet.QUERY_TYPE_UPDATE);
		instanceHttpClient.addParam(AndroidBeerQueryServlet.KEY_MAPPED_RATING, new JSONObject(rating).toString());
	...
	private static boolean processBooleanResponse(String response)
		...
		if (jsonObj.has(AndroidBeerQueryServlet.RESULT_SUCCESS)) {
			return jsonObj.getBoolean(AndroidBeerQueryServlet.RESULT_SUCCESS);
		}
